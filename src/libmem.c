...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  if (get_free_vmrg_area(caller, vmaid, size, &rgnode) == 0)
  {
    ...
    *alloc_addr = rgnode.rg_start;
    ...
    #ifdef DEBUG
  printf("=====2 PHYSICAL MEMORY AFTER ALLOCATION =====\n");
  ...
  //NEU KHONG THE RETURN 0 BEN TREN -> KHONG LAY DUOC
  //FREE VMRG_AREA -> MO RONG PHAM VI TIM KIEM (thong qu sbrk)
   ...
   */
  struct sc_regs regs;
  regs.a2 = (uint32_t) vmaid; //doi tuong quan ly bo nho can mo rong
  regs.a3 = (uint32_t) inc_sz; //khoang can mo rong
  regs.a1 = SYSMEM_INC_OP; //lenh mo rong
  int syscall_ret = syscall(caller, 17, &regs);
  if (syscall_ret < 0){
    return -1;
  } // khong mo rong vung nho thanh cong
 ...
 *alloc_addr = old_sbrk;
 ...
 if(enlist_vm_freerg_list((caller->mm),new_rg)) return -1;
 pthread_mutex_unlock(&mmvm_lock);

#ifdef DEBUG
  printf("=====1 PHYSICAL MEMORY AFTER ALLOCATION =====\n");
  printf("PID=%d - Region=%d - Address=%08x - Size=%d byte\n",caller->pid,rgid,*alloc_addr,size);
#ifdef PAGETBLE_DUMP
  print_pgtbl(caller, 0, -1);
#endif
...
int __free(struct pcb_t *caller, int vmaid, int rgid)
{
  ...
  // deleteSymrgtbl.rg_start = 0;
  // deleteSymrgtbl.rg_end = 0;
  // /*enlist the obsoleted memory region */
  // enlist_vm_freerg_list(cur_vma->vm_mm,&deleteSymrgtbl);
  
  printf("concac\n");

  struct vm_rg_struct *new_frrg = malloc(sizeof(struct vm_rg_struct));
  new_frrg->rg_start = caller->mm->symrgtbl[rgid].rg_start;
  new_frrg->rg_end = caller->mm->symrgtbl[rgid].rg_end;

  if(enlist_vm_freerg_list(caller->mm, new_frrg) < 0)
    return -1;  
  ...
  printf("PID=%d - Region=%d",caller->pid,rgid);
#ifdef PAGETBLE_DUMP
  print_pgtbl(caller, 0, -1);
#endif
#endif
...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  { /* Page is not online, make it actively living */
     ...
     */
    struct sc_regs regs;
    regs.a1 = SYSMEM_SWP_OP;//lenh
    regs.a2 = vicfpn;//so trang
    regs.a3 = swpfpn;//so trang
    int syscall_ret = syscall(caller, 17, &regs);
    if (syscall_ret < 0) return -1; // khong swap trang thanh cong
    /* SYSCALL 17 sys_memmap */
    ...
    */
    // regs.a1 = SYSMEM_SWP_OP;//lenh
    // regs.a2 = tgtfpn;//so trang
    // regs.a3 = vicfpn;//so trang
    ...
    if(__swap_cp_page(caller->active_mswp, tgtfpn, caller->mram, vicfpn) < 0)
      return -1; // khong swap trang thanh cong
...
int get_free_vmrg_area(struct pcb_t *caller, int vmaid, int size, struct vm_rg_struct *newrg)
{
  ...
  while(rgit != NULL)
  {
    ...
    {
      //neu chon duoc vung nho
      newrg->rg_start = rgit->rg_start;
      newrg->rg_end = newrg->rg_start + size;
      //xu ly de thu hep vung nhơ trong con lai
      //vi du can lay 100 tu [150;300] thi con lai [250,300] 
      //{
        // //truong hop van con
        rgit->rg_start = rgit->rg_start + size;
        //truong hop su dung het thi xoa di
        if ((rgit->rg_end - rgit->rg_start) == 0){
          if(rgit_prev == NULL){
